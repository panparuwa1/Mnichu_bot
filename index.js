...
const scammerSelections = new Map();
...
function inviteStatsFor(guildId, userId) {
  config.inviteStats[guildId] ||= {};
  config.inviteStats[guildId][userId] ||= { joins: 0, leaves: 0, fake: 0 };
  return config.inviteStats[guildId][userId];
}
...
async function refreshInviteCache(guild) {
  ...
  const invites = await guild.invites.fetch().catch((error) => {
    console.error("Nie udalo sie pobrac zaproszen:", error);
    return null;
  });
...
async function findUsedInvite(guild) {
  ...
  const invites = await guild.invites.fetch().catch((error) => {
    console.error("Nie udalo sie sprawdzic uzytego zaproszenia:", error);
    return null;
  });

  if (!invites) return null;

  const usedInvite = invites.find((invite) => (invite.uses || 0) > (previous.get(invite.code) || 0));
  inviteCache.set(guild.id, new Map(invites.map((invite) => [invite.code, invite.uses || 0])));
  return usedInvite || null;
}

function memberInviteKey(guildId, memberId) {
  return `${guildId}:${memberId}`;
}

function isLikelyFakeAccount(user) {
  return Date.now() - user.createdTimestamp < 7 * 24 * 60 * 60 * 1000;
}

function invitesEmbed(guild, targetUser, stats, requestedBy) {
  const activeInvites = Math.max(0, (stats.joins || 0) - (stats.leaves || 0) - (stats.fake || 0));
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        `**${targetUser}**`,
        "",
        `✅ **${stats.joins || 0}** osób dołączyło`,
        `❌ **${stats.leaves || 0}** osoby opuściły serwer`,
        `💩 **${stats.fake || 0}** fałszywych kont`,
        "",
        `Masz obecnie **${activeInvites}** aktywnych zaproszenia! 👏`,
      ].join("\n")
    )
    .setThumbnail(targetUser.displayAvatarURL({ size: 256 }))
    .setFooter({
      text: `${SETTINGS.shopName} • Asked by ${requestedBy.username}`,
...
// ===== SCAMERS - WYBOR UZYTKOWNIKA SERWERA =====
async function showScammerPicker(interactionOrMessage) {
  const select = new UserSelectMenuBuilder()
    .setCustomId("scammer_select")
    .setPlaceholder("Wybierz nazwe scamera z uzytkownikow serwera")
    .setMinValues(1)
    .setMaxValues(1);
  ...
  const embed = new EmbedBuilder()
    .setColor(COLOR)
    .setTitle("Scamers - wybierz osobe")
    .setDescription(
      [
        "Drugie pole, czyli nazwa scamera, jest teraz wyborem uzytkownika z serwera.",
        "Kliknij liste ponizej i wybierz osobe, ktora oszukala.",
        "",
        "Po wybraniu poprosze Cie o opis scamu.",
      ].join("\n")
    )
    .setFooter({ text: `${SETTINGS.shopName}` });

  const payload = { embeds: [embed], components: [rowSelect] };

  await interactionOrMessage.reply({ ...payload, ephemeral: true });
}

function showScammerDescriptionModal(interaction, pickedUser) {
  const modal = new ModalBuilder()
    .setCustomId("ticket_modal_scamers_description")
    .setTitle("Scamers - formularz".slice(0, 45));

  const idInput = new TextInputBuilder()
    .setCustomId("ticket_scammer_id")
    .setLabel("ID scamera")
    .setStyle(TextInputStyle.Short)
    .setValue(pickedUser.id)
    .setRequired(true);
  ...
  // Zapisz wybor w cache
  scammerSelections.set(interaction.user.id, {
    userId: pickedUser.id,
    username: pickedUser.username,
    displayName: pickedUser.displayName || pickedUser.globalName || pickedUser.username,
  });

  return interaction.showModal(modal);
}
// ===== KONIEC SCAMERS - WYBOR UZYTKOWNIKA =====
function showTicketModal(interaction, type) {
  ...
  if (type === "mystery-konta") {
    const accountInput = new TextInputBuilder()
      .setCustomId("ticket_mystery_account")
      .setLabel("Jakie konto?")
      .setStyle(TextInputStyle.Short)
      .setPlaceholder("np. Basic, Advanced, Pro, Legend")
      .setRequired(true);

    const priceInput = new TextInputBuilder()
      .setCustomId("ticket_mystery_price")
      .setLabel("Cena")
      .setStyle(TextInputStyle.Short)
      .setPlaceholder("np. 30zl, 50zl, 100zl, 300zl")
      .setRequired(true);

    const paymentInput = new TextInputBuilder()
      .setCustomId("ticket_mystery_payment")
      .setLabel("Metoda platnosci")
      .setStyle(TextInputStyle.Short)
      .setPlaceholder("np. BLIK, PayPal, PSC, Crypto")
      .setRequired(true);
  ...
  if (type === "scamers") {
    return showScammerPicker(interaction, 0);
  }
  if (type === "partnerstwa") {
...
function rulesEmbed(guild) {
  return new EmbedBuilder()
    ...
    .setImage("https://www.image2url.com/r2/default/images/1783013730962-02b70510-297b-498b-a458-d5e968fa7546.png") // <- tym razem daj dobre zdjecie plz
    .setFooter({
      text: `© 2026 ${SETTINGS.shopName}`,
      iconURL: guild.iconURL({ dynamic: true }),
    });
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("zapro")
    .setDescription("Pokazuje statystyki zaproszen uzytkownika")
    .addUserOption((option) =>
      option.setName("osoba").setDescription("Czyje zaproszenia pokazac").setRequired(true)
    ),
...
client.on("guildMemberAdd", async (member) => {
  ...
  if (inviterId) {
    const stats = inviteStatsFor(member.guild.id, inviterId);
    stats.joins += 1;

    if (isLikelyFakeAccount(member.user)) {
      stats.fake += 1;
    }

    config.memberInvites[memberInviteKey(member.guild.id, member.id)] = inviterId;
    saveConfig();
  } else {
...
client.on("guildMemberRemove", async (member) => {
  const inviterId = config.memberInvites?.[memberInviteKey(member.guild.id, member.id)];

  if (inviterId) {
    const stats = inviteStatsFor(member.guild.id, inviterId);
    stats.leaves += 1;
    delete config.memberInvites[memberInviteKey(member.guild.id, member.id)];
    saveConfig();
  }
...
await interaction.channel.send({
  ...
  return interaction.reply({
    ...
    if (interaction.commandName === "zapro") {
      ...
      const stats = inviteStatsFor(interaction.guild.id, targetUser.id);

      return interaction.reply({
        embeds: [invitesEmbed(interaction.guild, targetUser, stats, interaction.user)],
      });
    }
  ...
  if (
    ...
    const rawPickedId = interaction.values?.[0];
    const pickedId = rawPickedId?.startsWith("scammer_pick:")
      ? rawPickedId.replace("scammer_pick:", "")
      : rawPickedId;
    ...
    const pickedUser = pickedMember?.user || interaction.users.get(pickedId);
    if (!pickedUser) {
      return interaction.reply({ content: "Nie znaleziono tej osoby na serwerze.", ephemeral: true });
    }

    return showScammerDescriptionModal(interaction, {
      id: pickedUser.id,
      username: pickedUser.username,
      globalName: pickedUser.globalName,
      displayName: pickedMember?.displayName,
    });
  }

  if (interaction.isButton() && interaction.customId === "scammer_cancel") {
    scammerSelections.delete(interaction.user.id);
    return interaction.update({ content: "Anulowano wybor scamers.", embeds: [], components: [] });
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId.startsWith("ticket_modal_")) {
    ...
    if (type === "mystery-konta") {
      return createTicket(interaction, type, {
        accountType: interaction.fields.getTextInputValue("ticket_mystery_account"),
        price: interaction.fields.getTextInputValue("ticket_mystery_price"),
        payment: interaction.fields.getTextInputValue("ticket_mystery_payment"),
      });
    }

    if (type === "scamers_description") {
      const picked = scammerSelections.get(interaction.user.id);
      scammerSelections.delete(interaction.user.id);

      if (!picked) {
        return interaction.reply({
          content: "Nie znaleziono wczesniejszego wyboru osoby. Sprobuj ponownie otworzyc ticket Scamers.",
          ephemeral: true,
        });
      }

      return createTicket(interaction, "scamers", {
        scammerId: interaction.fields.getTextInputValue("ticket_scammer_id"),
        scammerName: picked.displayName,
        scammerUsername: picked.username,
